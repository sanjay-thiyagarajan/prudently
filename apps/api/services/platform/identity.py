...
Application Default Credentials inside the deployed sandbox, and — per AGENTS.md's Day 3
finding — every deployed agent actually runs as the single shared Reasoning Engine service
agent (`service-<project-number>@gcp-sa-aiplatform-re.iam.gserviceaccount.com`), not its own
...
# of the per-agent SA above — confirmed Day 3, see AGENTS.md's "Service accounts / IAM"
# section. Project number, not project ID, per Agent Engine's own resource-path convention.
